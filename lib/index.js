...
.then(axios.spread(function (leaderboardData, normalizeData, localizationData, linksData, groupsData) {
    ...
    let addCards = function(items) {
        ...
        cardHeader.append('a')
            .attr('href', d => links[d.project][d.key].source)
            .attr('target', '_blank')
            .classed('is-hidden', d => {
        ...
        const svg = cardContent.append('div')
            .classed('column is-6 distribution clickable', true)
            .append('svg')
            .attr('width', '66')
            .attr('height', '33');
        svg.each(function(d) {
            ...
            svgElement.append('g')
                ...
                .on("click", function() {
                    ...
                    const enlarged = svgElement.classed('enlarged'),
                        width = enlarged ? 66 : 200;
                    chart.width(width);
                    d3.select(svgElement.node().parentNode)
                        .classed('is-6', true)
                        .classed('is-12', false);
                    d3.select(svgElement.node().parentNode.parentNode)
                        .select('.column.score')
                        .classed('is-hidden', false)
                        .transition().duration(300)
                        .style('opacity', enlarged ? 1 : 0)
                        .on("end", function() {
                            d3.select(this).classed('is-hidden', !enlarged);
                            d3.select(svgElement.node().parentNode)
                                .classed('is-6', enlarged)
                                .classed('is-12', !enlarged);
                        });

                    svgElement.classed('enlarged', !enlarged)
                        .transition().duration(300)
                        .attr('width', width);
                    d3.select(this).call(chart.duration(300), highlight);
                })
                .call(chart, highlight);
        });

        cardContent.append('div')
            .classed('column is-4 has-text-centered is-size-5 score', true)
