...
.then(axios.spread(function (leaderboardData, normalizeData, localizationData, linksData) {
    ...
    let addCards = function() {
        ...
        const card = d3.select('#cards')
            ...
            .append('div')
            .classed('card', true)
            .classed('card-normalized', d => normalize[d] !== null)
        ...
        cardContent.append('div')
            ...
            .html(d => {
                return `<strong>Project</strong><br> ${getFeatureValue(d)}`
            });

        cardContent.append('div')
            .classed('column is-4', true)
            .html(d => {
                return `<strong>Lead</strong><br> ${getLeadValue(d)}`
            });

        cardContent.append('div')
            .classed('column is-4', true)
            .html(d => {
                return `<strong>Mean</strong><br> ${getMeanValue(d)}`
            });
        ...
        let startDrag = function() {
            ...
            d3.event.on("drag", d => {
                var underElement = d3.select(document.elementFromPoint(d3.event.x, d3.event.y));
                if (!underElement.empty() && underElement.classed('card')) {
                    underElement.classed('droppable', true);
                }
                dragCard.style('left', (d3.event.x - start[0]) + 'px')
                    .style('top', (d3.event.y - start[1]) + 'px');
            }).on("end", d => {
                dragCard.transition().duration(200).ease(d3.easeLinear)
