...
.then(axios.spread(function (leaderboardData, normalizeData, localizationData, linksData, groupsData) {
    ...
    let getCardTitle = function(feature) {
        const name = locales.retrieve(localization, feature);
        if (normalize[feature] === null) {
            return `<span class="card-title">${name}</span>`;
        }

        return `<span class="card-title">${name}</span><span class="break">&nbsp;/&nbsp;</span> <span class="card-normalize">${locales.retrieve(localization, normalize[feature])}</span>`;
    };
    ...
    let getFeatureTitle = function(feature) {
        ...
        return `${name} / ${locales.retrieve(localization, normalize[feature])}`;
    };
    ...
    let orderCards = function() {
        ...
        let items = _.sortBy(data, (d) => {
            ...
            if (order === 'feature') {
                return locales.retrieve(localization, d.key);
            }
            if (order === 'group') {
    ...
    let addCards = function(items) {
        ...
        cardHeader.append('a')
            .attr('href', d => currentProject === null ? `#${d.project}` : `#${d.key}`)
            .attr('title', d => currentProject === null ?
                locales.message("card-swap-project", [d.project]) :
                locales.message("card-swap-feature", [locales.retrieve(localization, d.key)]))
        ...
        cardHeader.append('a')
            ...
            .attr('target', '_blank')
            .attr('title', locales.message("source-link"))
            .classed('is-hidden', d => {
        ...
        cardContent.append('div')
            ...
            .html(d => {
                return `<strong>${locales.message("score-project")}</strong><br> ${getFeatureValue(d.key, null, d.project)}`;
            });

        cardContent.append('div')
            .classed('column is-4 clickable', true)
            .on("click", d => { location.hash = `#${getLeadProject(d.key)}`; })
            .html(d => {
                return `<strong>${locales.message("score-lead")}</strong><br> ${getLeadValue(d.key)}`;
            });

        cardContent.append('div')
            .classed('column is-4', true)
            .html(d => {
                return `<strong>${locales.message("score-mean")}</strong><br> ${getMeanValue(d.key)}`;
            });
    ...
    let setCurrentProject = function(project, hasProject) {
        if (!hasProject) {
            setCurrentFeature(project);
            return false;
        }
    ...
    let setCurrentFeature = function(feature) {
        if (typeof data[projects[0]][feature] === "undefined") {
            return;
        }
        currentProject = null;
...
locales.generateNavigation(d3.select('#languages'), '', 'lang');
